...
import { addTime, fromMs, minusTime, multiplyTime, timelineTime, toMs, } from '../helpers/relative-time';
...
var CanvasInstance = /** @class */ (function (_super) {
    ...
    CanvasInstance.prototype.init = function () {
        ...
        this._$fastForward.on('touchstart click', function (e) {
            ...
            var goToTime = addTime(_this.getClockTime(), 20);
            if (goToTime < end) {
        ...
        this._$fastRewind.on('touchstart click', function (e) {
            ...
            var goToTime = minusTime(_this.getClockTime(), 20);
            if (goToTime >= start) {
                return _this._setCurrentTime(goToTime);
            }
    ...
    CanvasInstance.prototype.getRangeTiming = function () {
        ...
        var durationObj;
        var start = 0;
        var end = timelineTime(this._compositeWaveform ? this._compositeWaveform.duration : -1);
        var duration = end;
        ...
        if (end === -1) {
            Logger.warn('Duration not found...', { start: start, end: end, duration: duration, durationObj: durationObj });
        }
        return {
            start: start,
            end: end,
    ...
    CanvasInstance.prototype._canvasClockUpdater = function () {
        if (AVComponent.newRanges && this.isVirtual()) {
            ...
            var startDate_1 = fromMs(this._canvasClockStartDate);
            Logger.log('CanvasInstance._canvasClockUpdater()', {
                startDate: startDate_1,
                advanceToTime: minusTime(this.now(), startDate_1),
            });
            var paused = this.timePlanPlayer.advanceToTime(minusTime(this.now(), startDate_1)).paused;
            if (paused) {
                this.pause();
            }
            return;
        ...
        var startDate = fromMs(this._canvasClockStartDate);
