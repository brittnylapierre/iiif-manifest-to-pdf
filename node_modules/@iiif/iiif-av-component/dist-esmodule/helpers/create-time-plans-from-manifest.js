...
import { addTime, canvasTime, minusTime, timelineTime } from './relative-time';
export function createTimePlansFromManifest(manifest) {
    var parseRange = function (range, rangeStack, startDuration) {
        var _a, _b, _c;
        if (rangeStack === void 0) { rangeStack = []; }
        if (startDuration === void 0) { startDuration = timelineTime(0); }
        var timePlan = {
            type: 'time-plan',
            canvases: [],
        ...
        for (var canvasIndex = 0; canvasIndex < rangeRanges.length; canvasIndex++) {
            ...
            if (typeof ro === 'string') {
                ...
                var rStart = canvasTime(parseFloat(start || '0'));
                var rEnd = canvasTime(parseFloat(end || '0'));
                var rDuration = timelineTime(rEnd - rStart);
