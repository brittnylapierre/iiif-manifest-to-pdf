...
describe('JavaScript->PDF value serialization', () => {
    it('should convert Unicode strings with braces to UTF16BE', () => {
        expect(serialize('(Hellö Wörld)')).toBe('<FEFF00480065006C006C00F60020005700F60072006C0064>');
    });
    ...
    it('should convert Uint8Arrays to a hex string', () => {
        expect(serialize(new Uint8Array([0, 16, 32, 64, 128, 256]))).toBe('<001020408000>');
    });
    it('should convert Dates to a PDF date string', () => {
        expect(serialize(new Date('01 Jan 1970 00:00:00 GMT'))).toBe('(D:19700101000000Z)');
    });
...
describe('PDF generation', () => {
    it('should initialize a PDF with the correct metadata', async () => {
        ...
        await pdfgen.setup();
        const imgBuf = await fs.promises.readFile(path.resolve(__dirname, './fixtures/wunder.jpg'));
        await pdfgen.renderPage('http://some.fixture', { width: 290, height: 400 }, [
            {
