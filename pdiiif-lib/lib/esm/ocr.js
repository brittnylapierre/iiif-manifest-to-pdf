...
import { isExternalWebResourceWithProfile, vault, } from './iiif.js';
let parser;
let TextType;
 ...
 */
export function parseHocr(id, hocrText, referenceSize) {
    ...
    if (pageSize[2] !== referenceSize.width ||
        ...
        if (scaledWidth !== referenceSize.width ||
            ...
            log.debug(`Differing scale factors for x and y axis: x=${scaleFactorX}, y=${scaleFactorY}`);
        }
 ...
 * @returns {object} the parsed OCR page
 */
export function parseAlto(id, altoText, imgSize) {
    ...
    for (const blockNode of doc.querySelectorAll('TextBlock')) {
        ...
        for (const lineNode of blockNode.querySelectorAll('TextLine')) {
            const line = {
                height: Number.parseInt(lineNode.getAttribute('HEIGHT') ?? '0', 10) *
                    scaleFactorY,
                text: '',
                width: Number.parseInt(lineNode.getAttribute('WIDTH') ?? '0', 10) *
                    scaleFactorX,
                spans: [],
                x: Number.parseInt(lineNode.getAttribute('HPOS') ?? '0', 10) *
                    scaleFactorX,
                y: Number.parseInt(lineNode.getAttribute('VPOS') ?? '0', 10) *
                    scaleFactorY,
            };
            ...
            for (const [textIdx, textNode] of textNodes.entries()) {
                ...
                const width = Number.parseInt(textNode.getAttribute('WIDTH') ?? '0', 10) *
                    scaleFactorX;
                let height = Number.parseInt(textNode.getAttribute('HEIGHT') ?? '0', 10) *
                    scaleFactorY;
                if (Number.isNaN(height)) {
                    height = line.height;
                }
                const x = Number.parseInt(textNode.getAttribute('HPOS') ?? '0', 10) *
                    scaleFactorX;
                let y = Number.parseInt(textNode.getAttribute('VPOS') ?? '0', 10) *
                    scaleFactorY;
                if (Number.isNaN(y)) {
                    y = line.y;
                }
 ...
 */
export function parseOcr(id, ocrText, referenceSize) {
    let parse;
...
const isAlto = (resource) => resource.format === 'application/xml+alto' ||
    resource.profile?.startsWith('http://www.loc.gov/standards/alto/');
/** Checks if a given resource points to an hOCR document */
const isHocr = (resource) => resource.format === 'text/vnd.hocr+html' ||
    resource.profile ===
...
export function getTextSeeAlso(canvas) {
    const seeAlsos = vault.get(canvas.seeAlso);
    return seeAlsos
        .filter(isExternalWebResourceWithProfile)
        .find((r) => isAlto(r) || isHocr(r));
}
export async function fetchAndParseText(canvas, annotations) {
    ...
    if (seeAlso) {
        ...
        return (parseOcr(seeAlso.id, markup, {
            width: canvas.width,
